...
def get_column_by_position(
    ...
    if position >= len(df.columns):
        raise ValueError(
            f"{label} does not contain the required column "
...
def normalize_key_part(series: pd.Series) -> pd.Series:
    """Normalize lookup key components."""
    return (
        series.astype(str)
        .str.strip()
...
def build_direct_key(
    ...
    """Concatenate key components directly without a separator."""
    ...
    try:
        ...
        if st.button("Process Retail File", type="primary"):
            try:
                ...
                # Raw value used to populate Retail Column Q:
                # Raw Vendor Store Cost Column N
                raw_value = get_column_by_position(
                    raw_df,
                    13,
                    "Raw Vendor Store Cost",
                )
                ...
                # Keep the first occurrence for duplicate lookup keys.
                raw_lookup = (
                    raw_lookup
                    .drop_duplicates(
                        subset="_lookup_key",
                ...
                # Retail Column Q is the 17th column,
                # which is index 16 in pandas.
                ...
                matched_value = retail_key.map(
                    raw_lookup["_value"]
                )

                # Populate Retail Column Q only when
                ...
                # Ensure unmatched and ineligible rows are blank.
                output_df.iloc[:, 16] = (
                    output_df.iloc[:, 16]
                    .fillna("")
                    .astype(str)
                ...
                st.error(
                    f"Processing error: {process_error}"
                )

    except Exception as read_error:
        st.error(
            f"File reading error: {read_error}"
        )
